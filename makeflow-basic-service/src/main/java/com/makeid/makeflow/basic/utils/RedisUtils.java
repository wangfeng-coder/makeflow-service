...
public class RedisUtils {
    ...
    public static Set<String> scan(String patten) {
        return redisTemplate.execute((RedisCallback<Set<String>>) connection -> {
            Set<String> result = new HashSet<>();
            try {
                Cursor<byte[]> cursor = connection.scan(ScanOptions.scanOptions().match(patten).count(1000).build());
                while (cursor.hasNext()) {
                    byte[] next = cursor.next();
                    result.add(new String(next, StandardCharsets.UTF_8));
                }
            } catch (Exception e) {
                log.error("redis execute scan error, cause：{}", e.getMessage(), e);
            }
            return result;
        });
    }
