...
package com.makeid.makeflow.workflow.eventbus;
...
final class SubscriberRegistry {
  ...
  private Multimap<Class<?>, Subscriber> findAllSubscribers(Object listener) {
    ...
    for (Method method : getAnnotatedMethods(clazz)) {
      ...
      FlowSubscribe annotation = method.getAnnotation(FlowSubscribe.class);
      if(annotation.sync() && bus instanceof  AsyncEventBus) {
          continue;
      }
      methodsInListener.put(eventType, Subscriber.create(bus, listener, method));
      log.info("发现可注册的事件【{}】,监听器方法【{}】bus 【{}】",eventType.getName(),method.getName(),bus.identifier());
    }
  ...
  private static ImmutableList<Method> getAnnotatedMethodsNotCached(Class<?> clazz) {
    ...
    for (Class<?> supertype : supertypes) {
      for (Method method : supertype.getDeclaredMethods()) {
        if (method.isAnnotationPresent(FlowSubscribe.class) && !method.isSynthetic()) {
